...
from . import agents, litebaoding, litebounce
...
_TASK_AGENT_DIR = {"litebounce": "bounce", "litebaoding": "baoding"}


def _variant_paths(task_id: str) -> dict[str, str]:
    agent_folder = _TASK_AGENT_DIR.get(task_id.lower(), task_id.lower())
    base = os.path.join(_AGENTS_DIR, agent_folder)
...
def _register_task(task_id: str, env_cls, cfg_cls) -> None:
    ...
    gym.register(
        id=task_id,
        entry_point=f"roto.tasks.shadow_lite.{task_id.lower()}:{env_cls.__name__}",
        disable_env_checker=True,
        kwargs=kwargs,
    )


_register_task("LiteBounce", litebounce.BounceEnv, litebounce.BounceCfg)
_register_task("LiteBaoding", litebaoding.BaodingEnv, litebaoding.BaodingCfg)
